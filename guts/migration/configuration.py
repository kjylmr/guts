...
For instance, two lvm configurations can be set by naming them in groups as
 ...
 [source1]
 conversion_dir='/opt/stack/data'
 ...

 [dest1]
 conversion_dir='/opt/stack/'
 ...

And the configuration group name will be passed in so that all calls to
configuration.conversion_dir within that instance will be mapped to the proper
named group.
...
class Configuration(object):
    ...
    def _ensure_config_values(self, opts):
        CONF.register_opts(opts, group=self.config_group)

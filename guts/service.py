...
from guts.wsgi import eventlet_server as wsgi
...
service_opts = [
    ...
    cfg.StrOpt('osapi_migration_listen',
               default="0.0.0.0",
               help='IP address on which OpenStack Migration API listens'),
    cfg.PortOpt('osapi_migration_listen_port',
                default=7000,
                help='Port on which OpenStack Migration API listens'),
    cfg.IntOpt('osapi_migration_workers',
               help='Number of workers for OpenStack Migration API service. '
...
class Service(service.Service):
    ...
    def create(cls, host=None, binary=None, topic=None, manager=None,
        ...
        :param topic: defaults to bin_name - 'guts-' part
        ...
        if not topic:
            topic = binary
        if not manager:
            subtopic = topic.rpartition('guts-')[2]
